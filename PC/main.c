...
// For trig
// ─────────────────────────────────────────────────────────────────────────────
fixint sin_lookup[ANGLE_MAX];
fixint cos_lookup[ANGLE_MAX];

// ─────────────────────────────────────────────────────────────────────────────
// SDL surfaces & texture data
// ─────────────────────────────────────────────────────────────────────────────
SDL_Surface *screen;
...
/*static inline void setPixel(int32_t x, int32_t y, uint8_t color) {
	uint8_t *dst = (uint8_t*)screen->pixels + (y * SCREEN_WIDTH) + x;
	*dst = color;
}

static inline void SetPixel16(int32_t x, int32_t y, int32_t colors) {
	int32_t index = y * SCREEN_WIDTH + x;
    ((uint16_t*)screen->pixels)[index >> 1] = (uint16_t)colors;
}
*/
...
// ─────────────────────────────────────────────────────────────────────────────
    ...
    #define BIGENDIAN_SYSTEM
#elif defined(__BYTE_ORDER__) && __BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__
    #define LITTLEENDIAN_SYSTEM
#else
    #error "Unknown endianness. Please define BIGENDIAN_SYSTEM or LITTLEENDIAN_SYSTEM."
#endif
...
static inline void SetPixel8_xe(int16_t color) {
    ...
    *fb_ptr = (pixel & 0x00FF) | (int16_t)(color);
#elif defined(LITTLEENDIAN_SYSTEM)
    // In little endian, the first byte is the lower byte
    *fb_ptr = (pixel & 0xFF00) | (int16_t)(color);
#endif
}
...
static inline int32_t fetchTextureColor16(int32_t u, int32_t v, int32_t du, int32_t dv, int texture_index) {
    ...
    int16_t color = (int16_t)(texture16[index1] ); // Assuming lower byte contains the color index
}
...
bool load_background(const char* filename, uint8_t* bg_array) {
    ...
    if (bg_surface->w != SCREEN_WIDTH || bg_surface->h != SCREEN_HEIGHT) {
        fprintf(stderr, "Background must be %dx%d.\n", SCREEN_WIDTH, SCREEN_HEIGHT);
        SDL_FreeSurface(bg_surface);
        return false;
    }
    if (bg_surface->format->BitsPerPixel != 8) {
        fprintf(stderr, "Background must be 8-bit.\n");
        SDL_FreeSurface(bg_surface);
        return false;
    }
    ...
    if (memcmp(bg_surface->format->palette->colors,
               screen->format->palette->colors,
               sizeof(SDL_Color) * screen->format->palette->ncolors) != 0) {
        fprintf(stderr, "Palette mismatch.\n");
        SDL_FreeSurface(bg_surface);
        return false;
    }
    memcpy(bg_array, bg_surface->pixels, SCREEN_WIDTH * SCREEN_HEIGHT);
    SDL_FreeSurface(bg_surface);
    return true;
}
...
int main(int argc, char** argv) {
    ...
    // Load texture
    SDL_Surface *tex_surface = IMG_Load("textures.png");
    if (!tex_surface) {
        fprintf(stderr, "Load texture error: %s\n", IMG_GetError());
        SDL_Quit();
        return 1;
    }
    if (tex_surface->w != 32 || tex_surface->h != 224) {
        fprintf(stderr, "textures.png must be 32x224.\n");
        SDL_FreeSurface(tex_surface);
        SDL_Quit();
        return 1;
    }
    if (tex_surface->format->palette) {
        SDL_SetPalette(screen, SDL_LOGPAL | SDL_PHYSPAL,
                       tex_surface->format->palette->colors,
                       0, tex_surface->format->palette->ncolors);
    } else {
        fprintf(stderr, "Texture has no palette.\n");
        SDL_FreeSurface(tex_surface);
        SDL_Quit();
        return 1;
    }
    memcpy(texture, tex_surface->pixels, 32 * 224);
    SDL_FreeSurface(tex_surface);
